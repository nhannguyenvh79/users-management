  ...
  switch (SortField) {
    case "first_name":
      if (sortBy && SortField && sortBy === "asc") {
        termUsers.sort((a, b) =>
          a[SortField].toUpperCase().localeCompare(b[SortField].toUpperCase())
        );
      } else if (sortBy && SortField && sortBy === "desc") {
        termUsers.sort((a, b) =>
          b[SortField].toUpperCase().localeCompare(a[SortField].toUpperCase())
        );
      }
      return termUsers;
...
usersRouter.post("/", (req, res) => {
  if (req.body) {
    ...
    users.push(data);
    console.log(data);
    res.json({ status: "success", message: "student data saved successfully" });
  } else {
